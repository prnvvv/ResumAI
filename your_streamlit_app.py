...
# =====================================================================
GROQ_API_KEY1 = "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"
GROQ_API_KEY2 = "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"
GROQ_API_KEY3 = "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"
...
if 'user_data_history' not in st.session_state:
    st.session_state.user_data_history = []
...
def create_user_dataframe(user_info, analysis_data, job_link, resume_filename):
    ...
    user_data = {
        ...
        'Job Link': job_link,
        'Match Percentage': f"{analysis_data.get('match_percentage', 0)}%",
        'Missing Keywords Count': len(analysis_data.get('missing_keywords', [])),
        'Improvements Suggested': len(analysis_data.get('improvement_suggestions', [])),
        'Top Strength': analysis_data.get('match_reasons', {}).get('strengths', ['N/A'])[0] if analysis_data.get('match_reasons', {}).get('strengths') else 'N/A',
        'Top Gap': analysis_data.get('match_reasons', {}).get('gaps', ['N/A'])[0] if analysis_data.get('match_reasons', {}).get('gaps') else 'N/A',
        'Technical Skills Score': f"{analysis_data.get('skill_categories', {}).get('technical_skills', 0)}%",
        'Soft Skills Score': f"{analysis_data.get('skill_categories', {}).get('soft_skills', 0)}%",
        'Industry Knowledge': f"{analysis_data.get('skill_categories', {}).get('industry_knowledge', 0)}%",
        'Education Fit': f"{analysis_data.get('skill_categories', {}).get('education_fit', 0)}%",
        'Certifications Recommended': len(analysis_data.get('recommended_certifications', [])),
        'Analysis Mode': user_info.get('analysis_mode', 'Standard Analysis')
    }
    
    st.session_state.user_data_history.append(user_data)
...
def display_all_user_data():
    ...
    if st.session_state.user_data_history:
        ...
        <div class="dataframe-container">
            <h2 class="section-title">📊 Analysis History & User Data</h2>
        """, unsafe_allow_html=True)
        
        df = pd.DataFrame(st.session_state.user_data_history)
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "Match Percentage": st.column_config.TextColumn("🎯 Match %", width="small"),
                "Job Link": st.column_config.LinkColumn("🔗 Job Link", width="medium"),
            }
        )
        ...
        with col1:
            avg_match = sum([int(d['Match Percentage'].rstrip('%')) for d in st.session_state.user_data_history]) / len(st.session_state.user_data_history)
            st.markdown(f"""
        ...
        with col2:
            total_analyses = len(st.session_state.user_data_history)
            st.markdown(f"""
            <div class="metric-card">
        ...
        with col3:
            total_improvements = sum([d['Improvements Suggested'] for d in st.session_state.user_data_history])
            st.markdown(f"""
            <div class="metric-card">
                <div class="metric-title">💡 Total Suggestions</div>
                <div class="metric-value">{total_improvements}</div>
            </div>
            """, unsafe_allow_html=True)
        
        csv = df.to_csv(index=False)
        st.download_button(
            label="📥 Download Complete Analysis History (CSV)",
            data=csv,
            file_name=f"resume_analysis_history_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
            mime="text/csv",
            use_container_width=True
        )
        
        json_data = json.dumps(st.session_state.user_data_history, indent=2)
        st.download_button(
            label="📥 Download Complete Analysis History (JSON)",
            data=json_data,
            file_name=f"resume_analysis_history_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
...
def main():
    ...
    with st.sidebar:
        ...
        with st.expander("👤 User Information", expanded=True):
            user_name = st.text_input("Full Name", placeholder="John Doe", key="user_name")
            user_email = st.text_input("Email Address", placeholder="john.doe@email.com", key="user_email")
            user_phone = st.text_input("Phone Number", placeholder="+1 234 567 8900", key="user_phone")
    ...
    if analyze_button:
        if not user_name or not user_email:
            ...
            <div class="warning-alert">
                <strong>⚠️ User Information Required</strong><br>
                Please provide your name and email address to proceed.
            </div>
            """, unsafe_allow_html=True)
            return
        ...
        if analysis_result:
            ...
            create_user_dataframe(user_info, analysis_result, job_link, uploaded_file.name)
